...
function createRoom() {
    ...
    onDisconnect(newRoomRef).remove().catch((err) => {
...
let jsMesBool = true;
function jsMes() {
    const jsMes = document.getElementById("snackbar-cs");
    if (jsMesBool == true) {
        jsMesBool = false;
        jsMes.classList.add("show");
        setTimeout(function(){ jsMes.classList.remove("show"); jsMesBool = true; }, 3000);
    }
}
...
onValue(newRoomRef, (snapshot) => {
    ...
    if (data && data.win !== undefined && data.win !== "none" && data.col !== undefined && data.col !== "none") {
        ...
        document.getElementById("winningMessageCont").classList.add("show");
        update(newRoomRef, {
            win: "none",
            col: "none"
        });
...
document.getElementById("winningMessageBtn").addEventListener("click", function() {
    window.location.reload();
    sendRestart();
});
